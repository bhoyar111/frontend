...
export default function CategoryList() {
  ...
  const isChildRoute = location.pathname !== "/subcategories";
  ...
  const actions = [
    {
      label: "Edit",
      icon: <Edit size={14} />,
      onClick: (row) => navigate(`/subcategories/edit/${row._id}`),
    },
    {
      label: "Delete",
      icon: <Trash2 size={14} />,
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await Service.getListSubCategories(reqData);
  ...
  const handleConfirmAction = async (actionName, actionValue) => {
    ...
    try {
      const response = await Service.deleteSubCategory(reqData.categoryId);
      if (response?.status === 200) {
